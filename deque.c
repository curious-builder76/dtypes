...
deque_t* deque_custom(size_t element_size,void* (*xmalloc)(size_t), void (*xfree)(void*)){
	...
	uint8_t* buff=xmalloc(memory_required);
	if(!buff){
		xfree(deque);
		return NULL;
	}
...
int deque_grow(deque_t* deque){
	...
	uint8_t* new_buff=deque->malloc(new_size);
	if(!new_buff){
		deque->oom=1;
		return 1;
	}
	size_t idx=0;
	for(;;idx++){
		uint8_t* element=deque_popleft(deque);
		if(element==NULL){
			break;
		}
		uint8_t* new_location=new_buff+(idx*deque->element_size);
...
int deque_append(deque_t* deque,void* obj){
	...
	uint8_t* location= deque->buff + (deque->element_size*deque->front);
...
void* deque_pop(deque_t* deque){
	...
	uint8_t* ret_point=deque->buff+ (index * deque->element_size);
	deque->front=index;
	deque->size--;
	return ret_point;
}
...
int deque_appendleft(deque_t* deque,void* obj){
	...
	uint8_t* location= deque->buff + (new_rear*deque->element_size);
	memcpy(location,obj,deque->element_size);
	deque->rear=new_rear;
	deque->size++;
	return 0;
...
void* deque_popleft(deque_t* deque){
	...
	uint8_t* location= deque->buff + (deque->element_size*deque->rear);
...
int deque_oom(deque_t* deque){
	return (int)deque->oom;
}
