...
int hashmap_grow(hashmap_t* map){
	map->oom=1; // Assume guilty unless proven innocent.
...
// Return a non zero number if the last operation was failed because of OOM (Out of memory).
int hashmap_oom(hashmap_t* map){
	return (int) map->oom;
}
